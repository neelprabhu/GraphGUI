...
for ii=2:N
    ...
    getOUT = transfer(data);
%     f = displayGraph(ALL(:,:,ii-1), data(ii-1).VALL, data(ii-1).EALL, 'off');
%     g = print(f,'-RGBImage');
%     imwrite(g,'LatestFrame.png')
    Displayer;
    if ii == 2
        % To pause the algorithm:
        fprintf('Waiting for user input...\n')
        pause; fprintf('Tracking algorithm has resumed...\n');
    end
